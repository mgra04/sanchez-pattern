...
export async function startToolcraftFrameProbe(
  ...
  await page.evaluate(() => {
    ...
    const tick = (now: number) => {
      ...
      // The first headless-browser RAF can be arbitrarily delayed before a
      // visible interaction starts. Use it as the baseline rather than
      // misclassifying browser wake-up latency as product jank.
...
export async function measureToolcraftInteraction(
  ...
  await waitForToolcraftAnimationFrames(page, 1);
...
export async function dragToolcraftSliderByLabel(
  ...
  // Four intermediate pointer positions still exercise continuous live updates
  // while avoiding a synthetic event rate far above a real hand-driven drag.
