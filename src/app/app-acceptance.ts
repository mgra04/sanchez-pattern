...
export const appProductReadiness: ToolcraftProductReadiness = {
  mode: "product",
  productName: "Sanchez Pattern",
  productSummary:
    "A deterministic SVG pattern editor with Base Grid, directional Gradient Phase, clustered multi-size Mosaic, and equilateral Triangle lattice methods, backed by a frame-filtered local shape library with curated Shape Collections and local SVG preparation tools.",
  requestedBehavior:
    "Browse, filter, and open Shape Collections; import SVG variants; configure reusable shape sources; generate Base, Gradient, Mosaic, or clipped equilateral Triangle grids; preserve deterministic method-aware history locally; export matching SVG plus PNG/JPG output; and normalize Figma triangle root frames without moving authored geometry.",
};

function makeProductControlAcceptance(
  target: string,
  componentType: string,
  ...
  return {
    automated: true,
    automatedTestName: "validates pattern controls and deterministic vector output",
    browser: true,
    browserTestName: "browser: edits pattern controls and renders vector output",
    componentType,
    evidence: "product-output",
    expectedObservable: `${target} updates runtime state and the deterministic pattern workflow.`,
    fixture: "default square 8 by 16 base-grid fixture",
    id: target,
    kind: "control",
    target,
    userAction: `Change ${target} and observe the corresponding pattern workflow result.`,
    ...overrides,
  };
}

const allCustomBehavior = "all-custom-control-behavior" as const;

export const appAcceptance: readonly ToolcraftComponentAcceptance[] = [
  makeProductControlAcceptance("pattern.method", "select", {
    optionCoverage: ["base-grid", "directional-phases", "multi-size-mosaic", "triangle-lattice"],
    expectedObservable:
      "Base shows Base Distribution, Gradient shows Direction and phases, and Mosaic shows its recipe; shared Grid and sources survive switching.",
    userAction:
      "Switch between Base, Gradient, and Mosaic, verify method-specific controls, and confirm shared Grid/source plus separate method settings are preserved.",
  }),
  makeProductControlAcceptance("pattern.triangle.config", "triangleConfig", {
    automatedTestName: "exports clipped Triangle slots with subpixel geometry",
    browserTestName: "browser: creates and restores a Triangle pattern",
    builtInFitCheck: {
      checkedBuiltIns: ["collectionActions", "actions", "segmented", "slider", "select", "imagePicker"],
      closestBuiltIn: "collectionActions",
      productObservable: "Full/Half role pools, separate distributions, topology count, Inner gap, and linked start/alternating mirrors determine one lattice.",
      whyInsufficient: "CollectionActions cannot represent two geometry-filtered source pools plus parity-dependent topology controls and derived subpixel dimensions as one atomic recipe.",
    },
    customControlCoverage: allCustomBehavior,
    expectedObservable: "Triangle exposes independent Full/Half pools, two automatic caps, separate Inner and Grid gaps, parity-aware mirroring including odd-topology vertical alternation within each row, deterministic Seed, and exact clipped lattice output.",
    fixture: "2 by 2 lattice with two Full triangles and two automatic Half caps per element",
    userAction: "Choose Triangle, add compatible Full and Half sources, set an odd Full count, enable Alternate rows and Alternate elements in row, create a checkerboard of vertical SVG transforms, restore it after reload, verify the odd-only control preserves its hidden value when switching parity, and export matching SVG plus image bytes.",
  }),
  makeProductControlAcceptance("pattern.mosaic.amountMode", "segmented", {
    automatedTestName: "validates Mosaic amount modes and deterministic packing",
    browserTestName: "browser: creates and restores a Mosaic pattern",
    optionCoverage: ["count", "coverage"],
    expectedObservable: "Mosaic makes Amount mode visible while Base and Gradient hide it; Count authors exact totals while Coverage reports requested and closest representable actual occupied area.",
    fixture: "three-size one-cluster Mosaic fixture",
    userAction: "Choose Mosaic, verify Amount mode is visible, switch Count and Coverage, edit values, commit each mode, and compare rendered tile counts; choose Base and verify it is hidden.",
  }),
  makeProductControlAcceptance("pattern.mosaic.config", "mosaicConfig", {
    automatedTestName: "validates Mosaic amount modes and deterministic packing",
    browserTestName: "browser: Mosaic Anchor coverage and Spread stay independent",
    builtInFitCheck: {
      checkedBuiltIns: ["collectionActions", "actions", "segmented", "slider", "select", "imagePicker"],
      closestBuiltIn: "collectionActions",
      productObservable: "Size pools, recursive frames, cluster allocations, Anchor coverage, and Spread determine visible Mosaic tile geometry and sources.",
      whyInsufficient: "CollectionActions cannot represent two linked growable collections, dynamic source pools, a per-level allocation matrix, derived Coverage, local anchors, perimeter feedback, and authoritative packing in one atomic recipe.",
    },
    customControlCoverage: allCustomBehavior,
    expectedObservable: "Mosaic shows the recipe editor while Base and Gradient hide it; Anchor coverage changes immediate local-anchor perimeter occupancy, Spread independently changes anchor looseness and balanced branch reach, and invalid recipes block Create.",
    fixture: "48 by 24 three-size multi-cluster Mosaic fixture",
    userAction: "Choose Mosaic and verify the recipe editor appears; add and remove sizes and clusters, assign pools, edit Count/Coverage, Anchor coverage, and Spread, inspect requested/Actual coverage, then create, restore, and export Mosaic; choose Base and verify the editor is hidden.",
  }),
  makeProductControlAcceptance("ui.canvasSizingHelp", "canvasSizingHelp", {
    builtInFitCheck: {
      checkedBuiltIns: ["code", "text"],
      closestBuiltIn: "text",
      productObservable: "Compact guidance connects runtime canvas sizing with pattern auto-fit.",
      whyInsufficient: "A text input edits state and cannot present non-editable contextual guidance.",
    },
    customControlCoverage: allCustomBehavior,
    evidence: "command-side-effect",
    expectedObservable: "The sizing explanation remains visible without changing pattern state.",
  }),
  makeProductControlAcceptance("pattern.selectedSourceId", "shapeLibrary", {
    browserTestName: "browser: filters and opens shape collections",
    builtInFitCheck: {
      checkedBuiltIns: ["imagePicker", "select", "actions", "collectionActions"],
      closestBuiltIn: "imagePicker",
      productObservable:
        "Selecting a nested family, semantic weight or form, and radius adds an embedded vector source to pattern.sources.",
      whyInsufficient:
        "ImagePicker cannot browse two catalog categories and then resolve nested per-family weight or form and radius metadata into one runtime source.",
    },
    customControlCoverage: allCustomBehavior,
    expectedObservable:
      "The library opens as a movable Toolcraft dialog, combines Base/Complex and semantic Frame filters, drills into filtered Shape Collections with matching/total counts, previews family variants, and Add shape appends the chosen SVG source to the canvas pattern.",
    userAction:
      "Open Shapes Library, switch Base/Complex and Frame filters, enter and leave a Shape Collection, select a member family with its weight or form and radius, add the shape, and verify the dialog closes while runtime state plus SVG pattern output change.",
  }),
  makeProductControlAcceptance("library.importForm", "shapeImportDetails", {
    builtInFitCheck: {
      checkedBuiltIns: ["text", "segmented", "actions", "collectionActions"],
      closestBuiltIn: "text",
      productObservable: "One validated transaction combines family metadata, explicit variant name, radius, and import.",
      whyInsufficient: "Separate built-ins cannot share filename prefill, cross-field validation, inline errors, and one atomic import.",
    },
    customControlCoverage: allCustomBehavior,
    evidence: "command-side-effect",
    expectedObservable:
      "Import accepts arbitrary SVG filenames, prefills recognized conventions, validates variant name, radius, and Family ID inline, then stores locally.",
    userAction:
      "Attach an arbitrarily named 24 by 24 SVG, enter explicit metadata, verify invalid variant and missing family errors, then import it.",
  }),
  makeProductControlAcceptance("library.upload", "fileDrop", {
    evidence: "media-lifecycle",
    expectedObservable:
      "Uploading or dropping SVG files attaches ordered runtime media for atomic import; thumbnail/file reorder changes the order consumed by import, clear removes files, and global Reset removes uploaded media because no default exists.",
    userAction:
      "Upload two SVGs, verify both files, reorder their thumbnails and confirm import consumes the new runtime media order, clear one, upload again, run global Reset, and verify uploaded media is removed.",
  }),
  makeProductControlAcceptance("pattern.sources", "patternSources", {
    builtInFitCheck: {
      checkedBuiltIns: ["collectionActions", "actions", "imagePicker", "select"],
      closestBuiltIn: "collectionActions",
      productObservable:
        "Source selection, order, removal, frame normalization, and per-source preview all update the deterministic grid and selected inspector.",
      whyInsufficient:
        "CollectionActions cannot combine per-item SVG previews, active inspector selection, directional ordering, guarded removal, and cross-item frame validation in one runtime list.",
    },
    customControlCoverage: allCustomBehavior,
    expectedObservable:
      "Selecting a source loads its inspector values; reordering changes deterministic assignment order, remove deletes it, and Normalize frames resolves base-grid validation.",
    userAction:
      "Add two shapes, select each source, reorder them, remove one, create a frame mismatch, then Normalize frames and verify runtime state and pattern output.",
  }),
  makeProductControlAcceptance("ui.selectedShapeEmpty", "emptySelection", {
    builtInFitCheck: {
      checkedBuiltIns: ["text", "collectionActions"],
      closestBuiltIn: "text",
      productObservable: "A conditional empty-state message replaces selected-source controls.",
      whyInsufficient: "A text input is editable and cannot represent a non-editable empty state.",
    },
    customControlCoverage: allCustomBehavior,
    expectedObservable: "No shape selected is visible when selection is empty and hidden when a source is selected.",
  }),
  makeProductControlAcceptance("pattern.source.variant", "shapeVariant", {
    builtInFitCheck: {
      checkedBuiltIns: ["select", "segmented", "imagePicker", "collectionActions", "actions"],
      closestBuiltIn: "select",
      productObservable:
        "Choosing a semantic weight or form first constrains the available radius choices and swaps the exact SVG geometry used by every matching grid cell.",
      whyInsufficient:
        "A flat Select cannot express the dependent two-step weight-or-form then radius choice while retaining family-specific semantic labels and variant SVG metadata.",
    },
    customControlCoverage: allCustomBehavior,
    expectedObservable:
      "The control is visible for a selected source and hidden when selection is empty; weight or form selection filters radius variants and swaps SVG geometry.",
    userAction:
      "Select a source, choose another weight or form and radius, and verify the source preview plus all matching pattern cells change.",
  }),
  makeProductControlAcceptance("pattern.source.frameWidth", "slider", {
    expectedObservable: "Frame width is visible for a selected source, hidden when selection is empty, and updates cell width.",
  }),
  makeProductControlAcceptance("pattern.source.frameHeight", "slider", {
    expectedObservable: "Frame height is visible for a selected source, hidden when selection is empty, and updates cell height.",
  }),
  makeProductControlAcceptance("pattern.source.scale", "slider", {
    expectedObservable:
      "Shape scale changes geometry padding inside each fixed grid-cell frame without changing frame size.",
    userAction: "Select a source and verify Shape scale is visible and changes padding; remove all sources and verify it is hidden.",
  }),
  makeProductControlAcceptance("pattern.source.rotation", "slider", {
    expectedObservable: "Source rotation is visible for a selected source, hidden when selection is empty, and rotates preview plus pattern cells.",
  }),
  makeProductControlAcceptance("pattern.source.weight", "slider", {
    expectedObservable:
      "Probability is visible for a selected source and hidden when selection is empty; it changes occurrence frequency only in Weighted distribution.",
  }),
  makeProductControlAcceptance("ui.shapeFillEmpty", "emptySelection", {
    builtInFitCheck: {
      checkedBuiltIns: ["text", "collectionActions"],
      closestBuiltIn: "text",
      productObservable: "A conditional empty-state message replaces source fill controls.",
      whyInsufficient: "A text input is editable and cannot represent a non-editable fill empty state.",
    },
    customControlCoverage: allCustomBehavior,
    expectedObservable: "No shape selected is visible when selection is empty and hidden when a source is selected.",
  }),
  makeProductControlAcceptance("pattern.source.fillMode", "segmented", {
    optionCoverage: ["solid", "gradient"],
    expectedObservable:
      "Solid makes Color visible and Gradient hidden; Gradient makes Gradient visible and Color hidden, and output uses the chosen fill.",
    userAction:
      "Choose Solid and verify Color is visible while Gradient is hidden, then choose Gradient and verify the inverse plus rendered SVG fill changes.",
  }),
  makeProductControlAcceptance("pattern.source.color", "color", {
    expectedObservable:
      "Color is visible in Solid mode, hidden in Gradient mode, and changes selected-source SVG fills.",
    userAction:
      "Choose Solid, verify Color is visible and change it; choose Gradient, verify Color is hidden, and inspect rendered pattern fill.",
  }),
  makeProductControlAcceptance("pattern.source.gradient", "gradient", {
    controlPartCoverage: "all-visible-parts",
    expectedObservable:
      "Gradient is visible in Gradient mode, hidden in Solid mode, and type, angle, stop position, color, and opacity update selected-source SVG definitions.",
    userAction:
      "Choose Gradient, verify the editor is visible, edit every gradient type, angle and each stop field, then choose Solid and verify it is hidden.",
  }),
  makeProductControlAcceptance("pattern.source.opacity", "slider", {
    expectedObservable: "Source opacity is visible for a selected source, hidden when selection is empty, and updates preview plus pattern cells.",
  }),
  makeProductControlAcceptance("pattern.baseGrid.rows", "slider"),
  makeProductControlAcceptance("pattern.baseGrid.columns", "slider"),
  makeProductControlAcceptance("pattern.baseGrid.gap", "slider"),
  makeProductControlAcceptance("pattern.baseGrid.distribution", "segmented", {
    optionCoverage: ["equal", "weighted"],
    expectedObservable:
      "Base makes Base distribution visible and Gradient hides it; Equal assigns sources uniformly while Weighted reads per-source Probability.",
    userAction:
      "Choose Base, verify Base distribution is visible and change Equal/Weighted; choose Gradient and verify it is hidden.",
  }),
  makeProductControlAcceptance("pattern.baseGrid.seed", "slider", {
    expectedObservable: "The same seed reproduces identical cell assignments and a changed seed creates a new arrangement.",
  }),
  makeProductControlAcceptance("pattern.baseGrid.autoFit", "switch", {
    evidence: "viewport-side-effect",
    expectedObservable:
      "Auto-fit resizes editable canvas width and height to rows, columns, matching source frames, and gap.",
  }),
  makeProductControlAcceptance("pattern.directionalPhases.angle", "slider", {
    expectedObservable:
      "Gradient makes Direction angle visible and Base hides it; 0 degrees points right, 90 degrees points up, and 30 degrees creates rising diagonal bands.",
    userAction:
      "Choose Gradient, verify Direction angle is visible, commit at 0, 90, and 30 degrees, then choose Base and verify it is hidden.",
  }),
  makeProductControlAcceptance("pattern.directionalPhases.phases", "directionalPhases", {
    builtInFitCheck: {
      checkedBuiltIns: ["collectionActions", "actions", "switch", "segmented", "slider"],
      closestBuiltIn: "collectionActions",
      productObservable:
        "Ordered phase shares and nested reusable-source assignments determine which source pool generates each directional grid cell.",
      whyInsufficient:
        "CollectionActions cannot combine per-phase selection, rename, reorder, proportional share, nested source assignment, distribution, unit conversion, minimum count, cross-item 100% validation, and outgoing boundary settings derived from the next ordered phase.",
    },
    customControlCoverage: allCustomBehavior,
    expectedObservable:
      "Gradient makes phases visible and Base hides them; users add, remove, rename, reorder, normalize, assign sources, choose Equal or Weighted per phase, and configure deterministic Previous or Next Transition Spread per adjacent boundary with max width and strength.",
    userAction:
      "Choose Gradient, configure a phase boundary, prove isolated Previous and Next cells, Seed replay, angle support, reorder ownership, disabled sharp output, and history restoration; then choose Base and verify phases are hidden.",
  }),
  makeProductControlAcceptance("actions.regenerate", "actions", {
    actionCoverage: ["regenerate"],
    expectedObservable: "New seed increments the seed and produces a different deterministic grid arrangement.",
  }),
  makeProductControlAcceptance("pattern.appearance.opacityDistribution", "opacityDistribution", {
    automatedTestName: "applies deterministic weighted opacity to whole shapes and separate SVG elements",
    browserTestName: "browser: configures deterministic pattern opacity distribution",
    builtInFitCheck: {
      checkedBuiltIns: ["collectionActions", "actions", "switch", "segmented", "slider"],
      closestBuiltIn: "collectionActions",
      productObservable:
        "One pattern-wide editor combines enablement, Whole Shape or Each Element scope, an appearance seed, a growable opacity/chance distribution, exact 100% validation, and deterministic SVG output.",
      whyInsufficient:
        "CollectionActions cannot combine weighted probability rows, cross-row total validation, deterministic seed replay, and unit-aware whole-versus-element rendering in one atomic setting.",
    },
    customControlCoverage: allCustomBehavior,
    expectedObservable:
      "Disabled preserves source appearance; enabled Whole Shape assigns one weighted opacity per cell, Each Element assigns one per annotated SVG unit, the same seed reproduces output, and invalid chance totals block pattern creation.",
    fixture: "equilateral triangle glyph sources with four automatically numbered opacity units",
    userAction:
      "Enable Opacity Distribution, switch Whole Shape and Each Element, edit opacity/chance rows to total 100%, create twice with the same seed, change the appearance seed, and inspect preview, history restoration, SVG export, and raster export.",
  }),
  makeProductControlAcceptance("pattern.name", "text", {
    expectedObservable: "Pattern name labels the next created entry or renames the active entry when it is updated.",
  }),
  makeProductControlAcceptance("actions.pattern", "actions", {
    actionCoverage: ["create-pattern", "update-pattern"],
    expectedObservable:
      "Create pattern validates the draft, appends and activates an immutable history snapshot, while Update pattern replaces only the active entry.",
    userAction:
      "Create a pattern, change its draft, create a second pattern, select the first, load it for editing, and update it without removing the second entry.",
  }),
  makeProductControlAcceptance("pattern.history", "patternHistory", {
    builtInFitCheck: {
      checkedBuiltIns: ["collectionActions", "actions", "select"],
      closestBuiltIn: "collectionActions",
      productObservable:
        "History activation swaps the exact visible snapshot while per-entry edit, rename, and delete actions coordinate draft and active output state.",
      whyInsufficient:
        "CollectionActions cannot render active snapshot state and coordinate activate, restore-to-draft, inline rename, and deterministic delete fallback per entry.",
    },
    customControlCoverage: allCustomBehavior,
    expectedObservable:
      "The newest or explicitly selected history entry is visible on canvas; Edit restores its draft, rename preserves it, and delete selects the newest remaining entry or clears output.",
  }),
  makeProductControlAcceptance("export.includeBackground", "switch", {
    evidence: "exported-bytes",
    expectedObservable:
      "Turning Include off hides the live preview product background and exports PNG with transparent alpha; video output would keep its product background.",
    userAction:
      "Toggle Include off, verify the preview background is hidden, export PNG and inspect transparent alpha, and verify video export policy keeps the background.",
  }),
  makeProductControlAcceptance("appearance.background", "color", {
    expectedObservable: "Background color updates the included preview rectangle and exported image background.",
  }),
  makeProductControlAcceptance("export.fileName", "text", {
    evidence: "exported-bytes",
    expectedObservable: "The sanitized file name is applied to SVG, PNG, and JPG downloads with the correct extension.",
  }),
  makeProductControlAcceptance("export.image.format", "select", {
    evidence: "exported-bytes",
    optionCoverage: ["png", "jpg"],
    expectedObservable: "PNG and JPG choices change exported MIME type and filename extension.",
  }),
  makeProductControlAcceptance("export.image.resolution", "select", {
    evidence: "exported-bytes",
    optionCoverage: ["2k", "4k", "8k"],
    expectedObservable: "2K, 4K, and 8K choices change the decoded exported image dimensions.",
  }),
  makeProductControlAcceptance("ui.exportHelp", "exportHelp", {
    builtInFitCheck: {
      checkedBuiltIns: ["text", "code"],
      closestBuiltIn: "text",
      productObservable: "Static guidance distinguishes shared file naming from raster-only format and resolution.",
      whyInsufficient: "An editable text control cannot present non-editable export guidance.",
    },
    customControlCoverage: allCustomBehavior,
    evidence: "command-side-effect",
    expectedObservable: "Export guidance states that image format and resolution do not affect SVG export.",
  }),
  makeProductControlAcceptance("actions.output", "panelActions", {
    actionCoverage: ["export-svg", "export-image"],
    evidence: "exported-bytes",
    expectedObservable:
      "Live preview validates matching frames; Export SVG creates portable vector bytes; Export Image uses filename, format, resolution, and background to create non-empty correctly sized bytes.",
    userAction:
      "Create a valid live pattern, export SVG, then export JPG at 8K and verify non-transparent pixels, MIME type, dimensions, filename, and background.",
  }),
  {
    automated: true,
    automatedTestName: "restores persisted pattern settings after reload",
    browser: true,
    browserTestName: "browser: restores persisted pattern settings after page reload",
    componentType: "runtime",
    evidence: "persistence-state",
    expectedObservable:
      "Changing rows and background, reloading the page, and reopening the app restores the edited values and matching SVG output.",
    fixture: "locally persisted edited base-grid fixture",
    id: "runtime.persistence.reload",
    kind: "runtime",
    persistenceCoverage: "reload",
    userAction:
      "Change rows and background, wait for local persistence, reload the browser page, and verify both controls and rendered pattern output are restored.",
  },
];

export const starterControlSectionInventory: readonly ToolcraftControlSectionInventoryEntry[] = [
  {
    entity: "canvas sizing guidance",
    groupingReason: "Pattern auto-fit and runtime-owned manual sizing need one concise explanation.",
    targets: ["ui.canvasSizingHelp"],
    title: "Canvas Sizing",
    workflowStage: "output sizing",
  },
  {
    entity: "pattern generation method",
    groupingReason: "The method selector stays with its gated Base distribution, Gradient phases, Mosaic recipe, and Triangle lattice controls as one generation-strategy dependency group.",
    targets: [
      "pattern.method",
    ...
    workflowStage: "method selection",
    splitReason: "Base distribution is method-gated here while shared grid geometry remains in the later Grid workflow section.",
  },
  {
    entity: "shape catalog",
    groupingReason: "Catalog browsing and adding one configured vector source is one discovery task.",
    targets: ["pattern.selectedSourceId"],
    title: "Shape Library",
    workflowStage: "source discovery",
  },
  {
    entity: "browser-local SVG source",
    groupingReason: "The runtime file lifecycle is kept in its required standalone upload section.",
    targets: ["library.upload"],
    title: "SVG file",
    workflowStage: "library extension",
  },
  {
    entity: "browser-local catalog metadata",
    groupingReason: "Family, variant, radius, category, validation, and import complete one transaction.",
    targets: ["library.importForm"],
    title: "Import Details",
    workflowStage: "library extension",
  },
  {
    entity: "active pattern sources",
    groupingReason: "Selection, order, removal, and frame compatibility operate on the source collection.",
    targets: ["pattern.sources"],
    title: "Pattern Shapes",
    workflowStage: "source composition",
  },
  {
    entity: "selected pattern source",
    groupingReason: "Variant, frame, internal scale, rotation, and probability all edit the selected source.",
    targets: [
      "ui.selectedShapeEmpty",
      "pattern.source.variant",
      "pattern.source.frameWidth",
      "pattern.source.frameHeight",
      "pattern.source.scale",
      "pattern.source.rotation",
      "pattern.source.weight",
    ],
    title: "Selected Shape",
    workflowStage: "source editing",
    splitReason: "Geometry and distribution properties precede the separate conditional fill workflow.",
  },
  {
    entity: "selected source appearance",
    groupingReason: "Fill mode gates solid or gradient controls while opacity applies to both.",
    targets: [
      "pattern.source.fillMode",
      "pattern.source.color",
      "pattern.source.gradient",
      "pattern.source.opacity",
      "ui.shapeFillEmpty",
    ],
    title: "Shape Fill",
    workflowStage: "source styling",
    splitReason: "Conditional solid and gradient editors form a distinct styling stage for the selected source.",
  },
  {
    entity: "shared pattern grid",
    groupingReason: "Grid dimensions, spacing, seed, fit, and regeneration apply to both generation methods.",
    targets: [
    ...
    title: "Grid",
    workflowStage: "pattern generation",
    splitReason: "Common grid geometry is shared by both methods while Base-only distribution stays with the method selector dependency group.",
  },
  {
    entity: "pattern-wide opacity variation",
    groupingReason: "Whole-shape or per-element probability rows and their independent seed jointly define one deterministic appearance pass.",
    targets: ["pattern.appearance.opacityDistribution"],
    title: "Opacity Distribution",
    workflowStage: "pattern styling",
  },
  {
    entity: "generated pattern artifact",
    groupingReason: "A name plus create and update actions commit the editable draft into one generated artifact.",
    targets: ["pattern.name", "actions.pattern"],
    title: "Pattern Output",
    workflowStage: "pattern generation",
  },
  {
    entity: "browser-local pattern history",
    groupingReason: "Activation, draft restoration, rename, and deletion operate on persisted pattern snapshots.",
    targets: ["pattern.history"],
    title: "Pattern History",
    workflowStage: "pattern management",
  },
  {
    entity: "product background",
    groupingReason: "Background inclusion and color jointly control preview and still export composition.",
    targets: ["export.includeBackground", "appearance.background"],
    title: "Background",
    workflowStage: "output styling",
  },
  {
    entity: "still image export",
    groupingReason: "Format and resolution configure the final raster export action.",
    targets: ["export.fileName", "export.image.format", "export.image.resolution", "ui.exportHelp"],
    title: "Image Export",
    workflowStage: "output setup",
  },
];
...
export function validateToolcraftAcceptanceCoverage(
  ...
  // Extra app acceptance rows are intentionally reusable in validator fixture tests.
  // Only the real app schema owns the invariant that every app-authored control row
  // must resolve to a currently visible product control.
  for (const entry of schema === appSchema ? acceptance : []) {
    if (
      entry.kind === "control" &&
      entry.target &&
